...
export class Controls{
    ...
    onWheel() {
        window.addEventListener("wheel", (e) => {
            if (!this.animationInProgress) {
                ...
                if (e.deltaY > 0) {
                    this.lerp.target = Math.min(this.lerp.target + 1 / 4, 0.99999);
                } else {
                    this.lerp.target = Math.max(this.lerp.target - 1 / 4, 0);
                }
    ...
    update(){
        ...
        /*this.house.catmulltab.forEach((value, index)=>{
            const lerpPosition = new THREE.Vector3(0, 0 ,0);
            value.getPointAt(this.lerp.current %1, lerpPosition);
            this.house.HouseGroup.children[index].position.copy(lerpPosition);
            //this.floor.floor.children[index].position.copy(lerpPosition);
        })*/
